...
extension TMDBClient {
    ...
    */
    ...
    func getWatchlistMovies(completionHandlerForWatchlist: (result: [TMDBMovie]?, error: NSError?) -> Void) {
        ...
        taskForGETMethod(mutableMethod, parameters: parameters) { (result, error) in
            ...
            guard let results = result[TMDBClient.JSONResponseKeys.MovieResults] as? [[String:AnyObject]] else {
                completionHandlerForWatchlist(result: nil, error: NSError(domain: "getWatchlistMofies parsing", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not parse getWatchlistMovies"]))
                return
            }
    ...
    func postToWatchlist(movie: TMDBMovie, watchlist: Bool, completionHandlerForWatchlist: (result: Int?, error: NSError?) -> Void) {
        ...
        var jsonBodyNS = NSString(string: "")
        do {
            jsonBodyNS =  try NSString(data: NSJSONSerialization.dataWithJSONObject(dataToSend, options: .PrettyPrinted), encoding: NSUTF8StringEncoding)!
        } catch {
            print(error)
        }
        print(jsonBodyNS)
        ...
        /* 2. Make the request */
        taskForPOSTMethod(mutableMethod, parameters: parameters, jsonBody: String(jsonBodyNS)) { (result, error) in
            /* 3. Send the desired value(s) to completion handler */
            guard (error == nil) else {
                completionHandlerForWatchlist(result: nil, error: error)
                return
            ...
            guard let results = result[JSONResponseKeys.StatusCode] as? Int else {
                completionHandlerForWatchlist(result: nil, error: NSError(domain: "postToWatchlist", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not parse postToWatchlist"]))
                return
            }
